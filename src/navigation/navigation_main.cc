...
void LaserCallback(const sensor_msgs::LaserScan& msg) {
  ...
  point_cloud_.clear();
  int right_point_idx = 0;
  int left_point_idx = 0;

  // Convert the LaserScan to a point cloud
  float range;
  float angle;
  Vector2f laser_loc;
  for (int i = 7; i < num_rays - 5; i++) {
    range = msg.ranges[i];
    if (range >= msg.range_min && range <= msg.range_max) {
      angle = msg.angle_min + i * msg.angle_increment;
      if (std::fabs(angle + M_PI / 2) < kEpsilon) {
        left_point_idx = i;
      }
      if (std::fabs(angle - M_PI / 2) < kEpsilon) {
        right_point_idx = i;
  ...
  navigation_->ObserveTrackWalls(point_cloud_, msg.header.stamp.toSec(), left_point_idx, right_point_idx);
